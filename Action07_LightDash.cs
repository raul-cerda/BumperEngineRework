...
public class Action07_LightDash : MonoBehaviour {
	...
	{
		InitialVelocityMagnitude = Player.rigidbody.velocity.magnitude;
    ...
    void FixedUpdate()
    {
		...
		if (Action.Action07Control.HasTarget) 
		{
			...
			Player.rigidbody.velocity = direction.normalized * DashSpeed;
		...
		if (!Action.Action07Control.HasTarget)
		{
			...
			Player.rigidbody.velocity = Vector3.zero;
			Player.rigidbody.velocity = direction.normalized*EndingSpeedResult*EndingSpeedFactor;
