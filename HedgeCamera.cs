...
public class HedgeCamera : MonoBehaviour {
	...
	void speedCam()
	{
		//original vals: max = -6,-20 @ .16  fov = 100, 60 @ .5
		float shiftSpeed = 60;
		if(Player.SpeedMagnitude >= shiftSpeed && CameraMaxDistance < -6)
			CameraMaxDistance += 0.16f;
		else if(Player.SpeedMagnitude < shiftSpeed && CameraMaxDistance >= -20)
			CameraMaxDistance -= 0.16f;
		if(Player.SpeedMagnitude >= shiftSpeed && Camera.main.fieldOfView < 100)
			Camera.main.fieldOfView += 0.5f;
		else if(Player.SpeedMagnitude < shiftSpeed && Camera.main.fieldOfView > 60)
			Camera.main.fieldOfView -= 0.5f;
	}
    ...
    void CameraMovement()
    {
        ...
        if (!Locked)
        {
			...
			//SensiX is always set to 0 and stops player camera control
            //x += (Input.GetAxis("Horizontal_right") * ((InputXSpeed) * SensiX)* InvertedX) * Time.deltaTime;
			x += (Input.GetAxis("Horizontal_right") * InputXSpeed* InvertedX) * Time.deltaTime;
            //y -= (Input.GetAxis("Vertical_right") * ((InputYSpeed) * SensiY) * InvertedY) * Time.deltaTime;
			y -= (Input.GetAxis("Vertical_right") * InputYSpeed * InvertedY) * Time.deltaTime;
            //x += (Input.GetAxis("Mouse X") * ((InputXSpeed) * SensiX) * InvertedX) * Time.deltaTime;
			x += (Input.GetAxis("Mouse X") * InputXSpeed * InvertedX) * Time.deltaTime;
            //y -= (Input.GetAxis("Mouse Y") * ((InputYSpeed) * SensiY) * InvertedY) * Time.deltaTime;
			y -= (Input.GetAxis("Mouse Y") * InputYSpeed * InvertedY) * Time.deltaTime;
        }
        else
        {
